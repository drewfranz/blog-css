...
options.styleGuide = {
  ...
  title: 'blog.drewfra.nz Style Guide'
};
...
// The default task.
// gulp.task('default', gulp.parallel('styles:production', 'styleguide', 'lint'));

// #################
// Build everything.
// #################
// gulp.task('build', gulp.series(['styles:production', 'styleguide', 'lint']));
...
gulp.task('lint:js-with-fail', gulp.series([], function () {
  return gulp.src(options.eslint.files)
    .pipe($.eslint())
    .pipe($.eslint.format())
...
gulp.task('lint:sass-with-fail', gulp.series([], function () {
  return gulp.src(options.theme.components + '**/*.scss')
    .pipe($.sassLint())
    .pipe($.sassLint.format())
...
gulp.task('clean:styleguide', gulp.series([], function () {
...
gulp.task('styleguide:kss-example-chroma', gulp.series([], function () {
...
// ##############################
gulp.task('watch:css', gulp.series(['clean:css', 'styles'], function () {
...
gulp.task('watch', gulp.series(['watch:css', 'watch:lint-and-styleguide',]));

// for running styleguide outside of drupal
gulp.task('watch-styleguide', gulp.series(['browser-sync', 'watch:lint-and-styleguide', 'watch:js']));
